...
{
	const Real radius = FLAGS_param_occlusion_test_neighbor_distance * _cuboid_structure.mesh_->get_object_diameter();
	...
	MeshCuboidStructure &_cuboid_structure)
{
	...
	//_cuboid_structure.null_cuboid_->clear_sample_points();
	...
	for (unsigned int point_index = 0; point_index < num_sample_points; ++point_index)
	{
		...
		{
			assert(_cuboid_structure.null_cuboid_);
			if (std::abs(sample_point->point_[0] - 0) < 1e-8 &&
				std::abs(sample_point->point_[1] - 0) < 1e-8 &&
				std::abs(sample_point->point_[2] - 0) < 1e-8)
				assert(false);

			if (sample_point->point_[0] > 1e8 ||
				sample_point->point_[1] > 1e8 ||
				sample_point->point_[2] > 1e8)
				assert(false);

			_cuboid_structure.null_cuboid_->add_sample_point(sample_point);
		}
	}
	...
	std::vector<MeshCuboid *>& _new_cuboids)
{
	...
	const unsigned int num_attributes = MeshCuboidAttributes::k_num_attributes;  /* 24 */
	unsigned int mat_size = num_cuboids * num_attributes;  /* num_cuboids * 24 */
	...
	for (unsigned int cuboid_index_1 = 0; cuboid_index_1 < num_given_cuboids; ++cuboid_index_1)
	{
		...
		assert(cuboid_1);
		LabelIndex label_index_1 = cuboid_1->get_label_index();  /* 给定(已有)部件的标签 */
		...
		for (unsigned int cuboid_index_2 = num_given_cuboids; cuboid_index_2 < num_cuboids; ++cuboid_index_2)
		{
			...
			assert(cuboid_2);
			LabelIndex label_index_2 = cuboid_2->get_label_index();  /* 缺失部件的标签 */
	...
	std::set<LabelIndex> &_ignored_label_indices)
{
	...
	std::set<LabelIndex> new_ignored_label_indices;
	for (std::set<LabelIndex>::const_iterator it = _ignored_label_indices.begin();
