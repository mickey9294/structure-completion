...
{
	/*FLAGS_data_root_path = "D:/Projects/shape2pose/";
	FLAGS_label_info_path = "data/0_body/assembly_airplanes/";
	FLAGS_mesh_path = "data/1_input/assembly_airplanes/off/";
	FLAGS_sample_path = "data/2_analysis/assembly_airplanes/points/even1000/";
	FLAGS_dense_sample_path = "data/2_analysis/assembly_airplanes/points/random100000/";
	FLAGS_mesh_label_path = "data/1_input/assembly_airplanes/gt/";
	FLAGS_sample_label_path = "data/4_experiments/exp1_assembly_airplanes/1_prediction/";
	FLAGS_training_dir = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/training";
	FLAGS_output_dir = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/output";
	FLAGS_symmetry_detection_dir = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/symmetry_detection";
	FLAGS_pose_filename = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/pose.txt";
	FLAGS_occlusion_pose_filename = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/pose.txt";
	FLAGS_part_assembly_dir = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/part_assembly";
	FLAGS_baseline_dir = "D:/Projects/structure-completion-master/experiments/assembly_airplanes/baseline";*/
	...
	bool _verbose)
{
	...
	std::string sample_label_filepath;
	if (_option != LoadPartAssemblyData)
	{
		sample_filepath = FLAGS_data_root_path + FLAGS_sample_path + std::string("/") + mesh_name + std::string(".pts");
		sample_label_filepath = FLAGS_data_root_path + FLAGS_sample_label_path + std::string("/") + mesh_name + std::string(".arff");
	}
	else
	{
		sample_filepath = FLAGS_output_dir + "/" + mesh_name + "/" + mesh_name + "_0_fusion.pts";
		sample_label_filepath = FLAGS_output_dir + "/" + mesh_name + "/" + mesh_name + "_0_fusion_label.arff";
	}
	...
	if (!(_option == LoadTestData || _option == LoadDenseTestData || _option == LoadPartAssemblyData) && !mesh_label_file.exists())
	{
	...
	if ((_option == LoadTestData || _option == LoadDenseTestData || _option == LoadPartAssemblyData) && !sample_label_file.exists())
	{
	...
	if (_option == LoadSamplePoints)
	{
		...
		{
			ret = _cuboid_structure.load_samples_label(mesh_label_filepath.c_str(), false);
			assert(ret);
		}
	}
	...
	else if (_option == LoadGroundTruthCuboids)
	{
		...
		std::string gt_file = FLAGS_training_dir + std::string("/") + mesh_name + std::string(".arff");
		cuboid_filepath_sstr << gt_file;
	}
	...
	else if (_option == LoadPartAssemblyData)
	{
		if (_verbose)
			std::cout << "- Load sample points for part assembly." << std::endl;
		...
		if (_verbose)
			std::cout << " - Load sample point labels for part assembly." << std::endl;
	...
	if (cuboid_filepath_sstr.str() != "")
	{
			...
			++sample_point_index)
		{
			...
			if (_cuboid_structure.mesh_->n_faces() > 0)
			{
				...
				label = _cuboid_structure.mesh_->property(_cuboid_structure.mesh_->face_label_, fh);
			}
			else
			{
				label = sample_point->label_;
			}
...
void MeshViewerCore::predict()
{
	...
	{
		std::cout << "occlusion_pose_filename is null." << std::endl;
	...
	std::cout << "modelview_matrix before original snapshot:" << std::endl;
	display_matrix(modelview_matrix());

	updateGL();
	snapshot_filename_sstr.clear(); snapshot_filename_sstr.str("");
	snapshot_filename_sstr << mesh_output_path << filename_prefix << std::string("origin");
	...
	std::cout << "modelview_mat after view snapshot:" << std::endl;
	display_matrix(modelview_matrix());
	...
	std::cout << "snapshot_modelview_mat before input snapshot:" << std::endl;
	display_matrix(snapshot_modelview_matrix);
	std::cout << "modelview_mat before input snapshot:" << std::endl;
	display_matrix(modelview_matrix());
	snapshot(snapshot_filename_sstr.str().c_str());
	...
	while (!cuboid_structure_candidates.empty())
	{
		...
		snapshot_filename_sstr << mesh_intermediate_path << filename_prefix
			...
			<< std::string("s_") << round << "_" << snapshot_index;
		snapshot(snapshot_filename_sstr.str().c_str());
		++snapshot_index;
		draw_cuboid_axes_ = true;

		//std::cout << "At the beginning, # of samples = " << cuboid_structure_.num_sample_points() << std::endl;
		...
		std::cout << "samples # = " << cuboid_structure_.num_sample_points() << "; cuboids samples # = " 
			<< cuboid_structure_.num_cuboids_sample_points() << std::endl;

		std::string null_samples_out_path = mesh_output_path + filename_prefix + "_" + std::to_string(output_count_);
		cuboid_structure_.save_null_sample_points_to_ply(null_samples_out_path.c_str());
		output_count_++;
		...
		std::cout << "After reconstruction, # of samples = " << cuboid_structure_.num_sample_points() << std::endl;
