	...
	const Real _xy_size, const Real _z_size, const Real _angle, const std::string _output_filename)
{
	...
	example_cuboid_structure.load_labels((FLAGS_data_root_path +
		FLAGS_label_info_path + FLAGS_label_info_filename).c_str());
	example_cuboid_structure.load_label_symmetries((FLAGS_data_root_path +
		FLAGS_label_info_path + FLAGS_label_symmetry_info_filename).c_str());
	example_cuboid_structure.load_symmetry_groups((FLAGS_data_root_path +
	...
	bool ret = load_object_info(example_mesh, example_cuboid_structure,
		_mesh_filepath.c_str(), LoadPartAssemblyData, NULL, false);
	assert(ret);
...
void MeshViewerCore::run_part_assembly()
{
	...
	ret = load_object_info(mesh_, cuboid_structure_, mesh_filepath.c_str(), LoadPartAssemblyData);
	if (!ret) return;
